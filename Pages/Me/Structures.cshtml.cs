...
namespace  wilhe1m.StructureWatch.Pages.Me
{
    ...
    public class structureModel : PageModel
    {
        ...
        {
            NotificationsAboutId = context.Notifications.Where(n => n.Text.Contains("//"+id+"\"")).OrderByDescending(n => n.Timestamp).ToList();
            ...
            if(Structure == null && NotificationsAboutId.Count>0 ){
                ...
                Structure.StructureId = long.Parse(Regex.Match(NotificationsAboutId[0].ParsedData["structureLink"],@"//(\d+)").Groups[1].Value);
